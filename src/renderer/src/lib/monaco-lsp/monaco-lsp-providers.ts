...
import type { IDisposable, IPosition, IRange, editor } from 'monaco-editor'
...
/** A numeric TS enum object → its name→value pairs (drops the reverse value→name entries). */
function numericEnumEntries(enumObject: object): Record<string, number> {
    ...
    monaco.languages.registerCompletionItemProvider(languageId, {
      ...
      provideCompletionItems: async (model, position) => {
        ...
        const converted = lspCompletionToMonaco(response.result, defaultRange, {
          kinds: numericEnumEntries(monaco.languages.CompletionItemKind),
        ...
        return {
          ...
          // numbers are assignable to Monaco's numeric enums, so no cast is needed.
          suggestions: converted.suggestions,
