...
function fakeModel(uri: string, text: string): FakeModel {
  let value = text
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the LSP document glue reads only uri, getValue, isDisposed and onDidChangeContent from a model.
  return {
    uri: { toString: () => uri },
    getValue: () => value,
    isDisposed: () => false,
...
function stubLspApi(): {
  ...
  Reflect.set(window, 'api', { lsp: api })
  return api
}
