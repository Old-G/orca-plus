...
import { asLspRecord, readLspArray, readLspString } from '../../shared/lsp-protocol-narrowing'
...
export function createLspSessionManager(deps?: {
  ...
  function handleServerMessage(session: LspSession, message: unknown): void {
    const parsed = asLspRecord(message)
    if (!parsed) {
      return
    }
    const id =
      typeof parsed.id === 'number' || typeof parsed.id === 'string' ? parsed.id : undefined
    const method = readLspString(parsed.method)
    const params = asLspRecord(parsed.params)
    if (id !== undefined && method === undefined) {
      const pending = session.pending.get(Number(id))
      if (!pending) {
        return
      }
      session.pending.delete(Number(id))
      clearTimeout(pending.timer)
      if (parsed.error !== undefined && parsed.error !== null) {
        const errorMessage = readLspString(asLspRecord(parsed.error)?.message)
        pending.reject(new Error(errorMessage ?? 'LSP request failed'))
      } else {
        pending.resolve(parsed.result ?? null)
      }
      return
    }
    const uri = readLspString(params?.uri)
    if (method === 'textDocument/publishDiagnostics' && uri) {
      ...
      const document = session.documentsByUri.get(canonicalFileUriKey(uri))
      if (!document) {
        return
      }
      for (const listener of diagnosticsListeners) {
        listener({
          ...
          diagnostics: readLspArray(params?.diagnostics) ?? []
        })
      }
      return
    }
    if (id !== undefined && method !== undefined) {
      ...
      const configurationItems = readLspArray(params?.items)
      const result =
        method === 'workspace/configuration' && configurationItems
          ? configurationItems.map(() => null)
          : null
      send(session, { jsonrpc: '2.0', id, result })
    }
  }
  ...
  function createSession(
    ...
    session.initialization = sendRequest(
      ...
      const capabilities = asLspRecord(asLspRecord(result)?.capabilities)
