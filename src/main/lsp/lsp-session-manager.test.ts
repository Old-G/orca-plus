...
import { asLspRecord } from '../../shared/lsp-protocol-narrowing'
...
function toJsonRpcMessage(raw: unknown): JsonRpcMessage {
  const record = asLspRecord(raw) ?? {}
  return {
    jsonrpc: '2.0',
...
function textDocumentField(message: JsonRpcMessage, field: string): unknown {
  return asLspRecord(asLspRecord(message.params)?.textDocument)?.[field]
}
...
function createFakeServer(serverCapabilities: Record<string, unknown> = {}) {
  // oxlint-disable-next-line typescript/consistent-type-assertions -- SAFETY: the manager only uses stdin/stdout/stderr/kill and events, all assigned right below.
  ...
  stdin.on('data', (chunk: Buffer) => {
    for (const message of decoder.push(chunk).map(toJsonRpcMessage)) {
...
describe('createLspSessionManager', () => {
  ...
  it('sends didChange with growing versions and didClose only at refcount zero', async () => {
    ...
    expect(changes.map((m) => textDocumentField(m, 'version'))).toEqual([2, 3, 4])
  ...
  it('announces .tsx documents as typescriptreact in didOpen', async () => {
    ...
    expect(textDocumentField(didOpen, 'languageId')).toBe('typescriptreact')
  })
